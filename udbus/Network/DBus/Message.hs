...
readHeader b = getWire endianess 0 getHeader b
	where getHeader = do
                _      <-                           getw8
		...
		return $! DBusHeader
			...
			, headerFieldsLength = flen
              ...
              ecode = B.head b
              endianess = if fromIntegral ecode /= fromEnum 'l' then BE else LE
...
readFields en b = getWire en 16 (getFields fieldsNew) b
	where
