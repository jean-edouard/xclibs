...
putValue (DBusByteArray content)  = do
        putw32 (fromIntegral $ B.length content)
        alignWrite (alignSigElement SigByte)
        putBytes content
...
getValue (SigArray SigByte) = do
        len <- fromIntegral <$> getw32
        alignRead (alignSigElement SigByte)
        DBusByteArray <$> getBytes len
